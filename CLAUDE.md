...
- `embeddings.py` — `extract_embeddings(paths, method)`. `"simple"` (default) is a deterministic offline color/edge descriptor; `"clip"` is real CLIP image embeddings (`clip_image_embeddings`, transformers ViT-B/32, batched, GPU-aware, cached) for semantic FP clustering — needs the `clip` extra (`uv sync --extra clip`, pinned `transformers<5`).
...
- `detection_eval.py` — `detection_metrics` (Ultralytics val → mAP@50, mAP@50-95, per-class recall) and `false_positive_rate` (negative-image FP-rate + mean FPs/image) for the detection gate.
- `gating.py` — `evaluate_promotion`: pure multi-criteria gate (mAP no-regression + per-class recall floor + FP-rate non-increase, with tolerances). Strengthens the registry's single-scalar gate; the key property is that an underfit "detect-nothing" model can't pass on FP-rate alone. Used by script 13.
- `dataset_builder.py` — `build_hard_negative_dataset` (confirmed-FP source frames → validated YOLO hard-negative dataset; empty labels for background) and `validate_label_lines` (drop corrupt/out-of-bounds labels). Closes the real detector loop with scripts 14 (build) → 15 (fine-tune on base+hard-negatives) → 13 (gate).
